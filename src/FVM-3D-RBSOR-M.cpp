...
#define n1 100        	/*number of divisions in R direction*/
#define n2 100        	/*number of divisions in B direction*/
#define n3 100          /*number of divisions in L direction*/
...
#pragma omp parallel for private(k,pom)
for (i=1;i<=n1;i++)
for (k=1;k<=n3;k++)
	{
...
#pragma omp parallel for private(j,pom)
for (i=1;i<=n1;i++)
for (j=1;j<=n2;j++)
	{
...
#pragma omp parallel for private(j,k,pom,pom1) reduction(+:res) reduction(+:res3)
	for (i=1;i<=n1;i++)
		for (j=1;j<=n2;j++)
			for (k=1;k<=n3;k++)
 			{
...
#pragma omp parallel for private(j,k) reduction(+:pom)
for (i=1;i<=n1;i++)
     for (j=1;j<=n2;j++)
       for (k=1;k<=n3;k++)
        {
